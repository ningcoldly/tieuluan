...
int XoaSachbangma(Sach ds[],int n, int maSachCanXoa, char tenSachCanXoa[]) {
    int found = 0; // bien kiem tra co tim thay sach khong
    for (int i = 0; i < n; i++) {
        if (ds[i].MaSach == maSachCanXoa && strcmp(ds[i].TenSach, tenSachCanXoa) == 0) {
            found = 1;
            for (int j = i; j < n - 1; j++) {
...
do  { 
   ...
   timkiemsach(ds, n, MaSachCanTim, tenSachCanTim, tacGiaCanTim, namXuatBanCanTim); // tim kiem sach theo Ma Sach, ten, tac gia, nam xuat ban
    ...
    n = XoaSachbangma(ds, n, maSachCanXoa, tenSachCanXoa);
    thongtinSach(ds, n);
            break;
        case '4':
           thongtinSach(ds, n);
